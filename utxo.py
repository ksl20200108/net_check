...
class UTXOSet(Singleton):
    ...
    def clear_transactions(self, transactions): # change 6.21
        used_uxto = []
        txs = []
        for tx in transactions: # avoid double spending
            try:  # change
                txid = tx.txid
                for vin in tx.vins:
                    vin_txid = vin.txid
                ...
                tx = Transaction.deserialize(tx)    # change deserialize successfully ! 6.22
                txid = tx.txid
                for vin in tx.vins:
                    vin_txid = vin.txid
                    uxto = (vin_txid, vin.vout)
